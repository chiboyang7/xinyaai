...
const PerfectWritingTask = () => {
  ...
  useEffect(() => {
    const currentTask = perfectWritingTasks.find((t) => t.id === taskId);
    if (currentTask) {
      setTask(currentTask);
      ...
      const savedProgress = localStorage.getItem(`perfectWritingTask_${taskId}`);
      if (savedProgress) {
        setUserInput(savedProgress);
      }
      const savedAI = localStorage.getItem(`perfectWritingTask_${taskId}_ai`);
      if (savedAI) {
  ...
  const handleSubmit = async () => {
    ...
    localStorage.setItem(`perfectWritingTask_${taskId}_ai`, response);
    setIsLoading(false);
    toast({
      title: "AI反馈已生成",
  ...
  const handleSaveProgress = () => {
    localStorage.setItem(`perfectWritingTask_${taskId}`, userInput);
    toast({
      title: "进度已保存",
  ...
  const handleComplete = () => {
    const savedTasks = localStorage.getItem("perfectWritingTasks");
    const tasks = savedTasks ? JSON.parse(savedTasks) : perfectWritingTasks;
    const updatedTasks = tasks.map((t: Task) =>
      t.id === taskId ? { ...t, completed: true } : t
    );
    localStorage.setItem("perfectWritingTasks", JSON.stringify(updatedTasks));
    setIsCompleted(true);
    toast({
      title: "任务完成！",
      description: "继续下一个任务",
  ...
  const handleNextTask = () => {
    const currentIndex = perfectWritingTasks.findIndex((t) => t.id === taskId);
    if (currentIndex < perfectWritingTasks.length - 1) {
      const nextTask = perfectWritingTasks[currentIndex + 1];
      navigate(`/perfect-writing/task/${nextTask.id}`);
    } else {
      navigate("/perfect-writing");
    }
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 mt-20">
        <Button
          variant="ghost"
          className="mb-6"
          onClick={() => navigate("/perfect-writing")}
        >
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex justify-between">
            <Button variant="outline" onClick={() => navigate("/perfect-writing")}>
              返回列表
            </Button>
            <div className="flex gap-2">
              {!isCompleted && (
...
export default PerfectWritingTask;
